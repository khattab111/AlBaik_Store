@php
    ...
    if (isset($product->pivot) && isset($product->pivot->discount_type)) {
        $salePivot = $product->pivot;
    } elseif ($product->relationLoaded('flashSales')) {
        $salePivot = $product->flashSales->first()?->pivot;
    }

    if ($salePivot && isset($salePivot->discount_type)) {
        $compareAt = $price;
        $price = $salePivot->discount_type === 'fixed'
            ? max(0, $price - (float) $salePivot->discount_value)
            : max(0, $price - ($price * ((float) $salePivot->discount_value / 100)));
    }

    $isHighlighted = $compareAt || (bool) ($product->is_featured ?? false);
@endphp

<article class="group overflow-hidden rounded-3xl border bg-white shadow-sm transition duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-red-950/10 {{ $isHighlighted ? 'border-red-200 shadow-red-950/5' : 'border-slate-200 hover:border-red-100' }} {{ $isList ? 'grid gap-0 sm:grid-cols-[220px_1fr]' : '' }}">
    <a href="{{ route('products.show', $product->slug) }}" class="relative block overflow-hidden bg-gradient-to-br from-red-50 via-white to-slate-100 {{ $isList ? 'min-h-56 sm:min-h-full' : 'aspect-[5/4]' }}">
        <img src="{{ $imageUrl }}" alt="{{ $product->name }}" loading="lazy" decoding="async" class="h-full w-full object-cover transition duration-500 group-hover:scale-105">
        <div class="absolute start-4 top-4 flex flex-wrap gap-2">
            ...
            @if ($compareAt)
                <span class="rounded-full bg-red-700 px-3 py-1 text-xs font-black text-white shadow-sm">{{ __('Sale') }}</span>
            @endif
        </div>
        <span class="sr-only">
            {{ $product->stock_quantity > 0 ? __('In Stock') : __('Out of Stock') }}
    ...
    <div class="grid gap-4 p-5 {{ $isList ? 'sm:p-6' : '' }}">
        <div>
            <p class="text-xs font-black uppercase tracking-normal text-red-700">{{ $product->brand?->name ?: 'ALBAIK' }}</p>
        ...
        <div class="flex items-end justify-between gap-3">
            ...
            <a href="{{ route('products.show', $product->slug) }}" class="text-sm font-black text-slate-500 hover:text-red-700" aria-label="{{ __('View details for :product', ['product' => $product->name]) }}">{{ __('Details') }}</a>
        </div>
